...
export class SidebarToggleDirective implements OnInit {
  @Input('appSidebarToggler') breakpoint: string | undefined;
  public bp:any;
...
export class AsideToggleDirective implements OnInit {
  @Input('appAsideMenuToggler') breakpoint: string | undefined;
  public bp:any;
  constructor(private classToggler: ClassToggler) {}
  ngOnInit(): void {
    this.bp = this.breakpoint;
...
export class HtmlAttributesDirective implements OnInit {
  ...
  private addClass(classes:any) {
    ...
    classArray.filter((element: string | any[]) => element.length > 0).forEach((element: string) => {
